...
void cadastre_update(int mode, int min_habitation, int max_habitation, int tempo, vec3 world, PImage map, boolean use_altitude_is) {
  ...
  if(mode == 0) {   
    if(new_commune) {
      cadastre_random_generator(num, SURFACE_HOME, world.xz(), map, use_altitude_is);
      new_commune = false;
    }
  } else if(mode == 1) {
    if(new_commune) {
      new_commune = false; 
      cadastre_map_generator(num, SURFACE_HOME, world.xz(), map, use_altitude_is, segment_monde);
    }
  }
}
...
void cadastre_map_generator(int num, int average_surface, vec2 flat_world, PImage map, boolean use_altitude_is, ArrayList<R_Line2D> segment) {
  int already_occupy = 0;
  for(int i = 0 ; i < num ; i++) {
