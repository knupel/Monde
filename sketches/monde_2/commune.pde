...
* LOT
*/
vec2 lot_street_map(int average_surface, vec2 flat_world, ArrayList<R_Line2D> segment) {
  int target = floor(random(segment.size()));
  vec3 a = segment.get(target).pointer_a().copy(); // need use copy to avoid any pointer problem
  vec3 b = segment.get(target).pointer_b().copy(); // need use copy to avoid any pointer problem
  float dist = a.mag(b);
