...
public void draw() {
	...
	if(mousePressed) {
		offset = map(mouseX,0,width,.1f,1);
...
public void show_text(int canvas) {
	textAlign(CENTER);
	textSize(16);
	fill(r.BLACK);
  int count =0;
...
public void show_point(int canvas) {
  strokeWeight(10);
  stroke(r.BLOOD);
  noFill();
	for(vec4 v : grid.get()) {
...
public void show_shape(float ratio_canvas, float ratio_cell) {
  strokeWeight(1);
  stroke(r.BLOOD);
  noFill();
  //fill(r.BLACK);
...
public class Sol {
	...
	public void set_altitude(int z) {
		this.coord.z = z;
	}

	public ivec3 get_coord() {
		return coord;
	}
...
abstract class Grid {
	...
	public ivec3 get_canvas_master() {
		return canvas.copy();
	}

	public ivec3 get_cage_master() {
		return cage.copy();
	}
...
public class Grid2D extends Grid {
  ...
  public void type_hex(int w, int h, int cell_x, int cell_y, float start_angle) {
	...
	public void type_hex(ivec2 canvas, ivec2 cell, float start_angle) {
	  ...
	  float median = side *(sqrt(3) *.5f); // find the length of the mediane equilateral triangle
    ...
    float inc_x = 1.f / get_cage().x;
		float inc_y = 1.f / get_cage().y;
    float offset_normal_x = inc_x *.5f;
		float offset_normal_y = inc_y *.5f;
...
public void print_list_costume() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "surface" };
    if (passedArgs != null) {
