...
public class generateur_de_monde extends PApplet {
...
public void init_map_relief() {
	relief_map = pattern_noise(width,height,.003f);
}
...
public void boussole(vec2 pos, int size) {
...
public void map(boolean show_info) {
	vec2 area = vec2(10);
...
add intersection
*/
public boolean ask_intersection() {
	int min_branch = 2;
	int max_branch = 5;
...
public boolean ask_intersection(Urbanist urb, int max_branch) {
...
public void add_intersection() {
	grid_nodes_monde.add(temp_intersection);
}



public boolean add_segment(Urbanist urb, boolean build_anytime, boolean show_info_is) {
	boolean add_is = false;
...
public int rank_intersection(Urbanist urb, vec target) {
	int rank = -1;
...
public int num_branch_by_intersection(int min, int max) {
...
*/
public vec2 goto_next(Urbanist urb, vec6 canvas, ArrayList<R_Node> inter_list, ArrayList<R_Segment> seg_list, boolean show_info) {
...
public boolean check_meeting_segment(R_Segment target_segment, ArrayList<R_Segment> seg_list, boolean show_info) {
	boolean is = false;
...
public vec2 compute_pos(Urbanist urb, vec6 canvas, ArrayList<R_Node> inter_list) {
...
public void intersection_is(boolean is) {
	intersection_is = is;
}
...
public void commune(vec3 world, int surface_habitation, int which_one) {
...
public void show_home(Home h, vec3 pos, int which) {
  if(which == 0) {
    h.show(pos);  
  } else {
    strokeWeight(1);
...
ArrayList<Home>town;
public void urbanisme(int surface, int max_level) {
  if(town == null) {
  ...
  if(town.size() != cadastre.size()) {
    ...
    for(int i = 0 ; i < cadastre.size() ; i++) {
      ...
      float peak = random(1);
      peak *= peak;
      h.set_architecture(vec2(peak*.5f));
...
public void init_commune_street_map() {
  new_commune = true;
}

public void init_cadastre() {
  if(cadastre == null) {
...
public void cadastre_update(int mode, int min_habitation, int max_habitation, int tempo, vec3 world, PImage map, boolean use_altitude_is) {
...
*/
public void cadastre_random_generator(int num, int average_surface, vec2 flat_world, PImage map, boolean use_altitude_is) {
...
public void cadastre_map_generator(int num, int average_surface, vec2 flat_world, PImage map, boolean use_altitude_is, ArrayList<R_Segment> segment) {
  int already_occupy = 0;
  for(int i = 0 ; i < num ; i++) {
...
*/
public void set_cadastre(vec2 pos, PImage map, int id, boolean use_altitude_is) {
  float altitude = 0;
...
* LOT
*/
public vec2 lot_street_map(int average_surface, vec2 flat_world, ArrayList<R_Segment> segment) {
...
public vec2 lot_random(int average_surface, vec2 flat_world) {
...
public boolean cadastre_is(vec2 pos, int average_surface, vec2 flat_world) {
...
public class Home {
  ...
  public Home(PApplet pa, int surface, float from_center, int max_level) {
    ...
    float add_length = avg_side* .75f;
    float max_side = avg_side + random(add_length);

    float x = random(avg_side,max_side);
    float y = 2*avg_side -x;

    float center_ratio = 1.f -from_center;
    if(center_ratio < .1f) center_ratio = .1f;
  ...
  public int get_fill_roof() {
    return this.fill_roof;
  }

  public int get_fill_wall() {
    return this.fill_wall;
  }

  public int get_fill_ground() {
...
public void plot(vec2 pos, int colour) {
	plot((int)pos.x(), (int)pos.y(), colour, 1.0f, g);
}

public void plot(vec2 pos, int colour, PGraphics pg) {
	plot((int)pos.x(), (int)pos.y(), colour, 1.0f, pg);
}

public void plot(vec2 pos, int colour, float alpha) {
	plot((int)pos.x(), (int)pos.y(), colour, alpha, g);
}

public void plot(vec2 pos, int colour, float alpha, PGraphics pg) {
...
public void set_pattern_no_angle() {
  init_pattern();
  rope_pattern.set_no_angle();
}

public void set_pattern_no_increment() {
  init_pattern();
  rope_pattern.set_no_increment();
}

public void set_pattern_no_smooth() {
  init_pattern();
  rope_pattern.set_no_smooth();
}

//setting
public void set_pattern_turbulence(float turbulence) {
  init_pattern();
  rope_pattern.set_turbulence(turbulence);
}

public void set_pattern_size(int w, int h) {
  init_pattern();
  rope_pattern.set_size(w,h);
}

public void set_pattern_range(float min, float max) {
  init_pattern();
  rope_pattern.set_range(min,max);
}


public void set_pattern_increment(float inc) {
  init_pattern();
  rope_pattern.set_increment(inc);
}

public void set_pattern_increment(float x, float y, float z) {
  init_pattern();
  rope_pattern.set_increment(x,y,z);
}

public void set_pattern_smooth(float smooth) {
  init_pattern();
  rope_pattern.set_smooth(smooth);
}

public void set_pattern_angle(float a_x, float a_y, float a_z) {
  init_pattern();
  rope_pattern.set_angle(a_x, a_y, a_z);
}

public void set_pattern_period(float x, float y) {
  init_pattern();
...
// build
public PGraphics pattern_rand(int w, int h) {
  init_pattern();
  rope_pattern.build_matrix_rand_mono();
  return rope_pattern.map_mono(w, h);
}

public PGraphics pattern_rand_xyz(int w, int h) {
  init_pattern();
  rope_pattern.build_matrix_rand_xyz();
  return rope_pattern.map_xyz(w, h);
}

public PGraphics pattern_noise(int w, int h) {
  init_pattern();
  rope_pattern.build_matrix_noise_mono();
  return rope_pattern.map_mono(w, h);
}

public PGraphics pattern_noise_xyz(int w, int h) {
  init_pattern();
  rope_pattern.build_matrix_noise_xyz();
  return rope_pattern.map_xyz(w, h);
}

public PGraphics pattern_img(PImage src, int w, int h) {
  init_pattern();
  rope_pattern.build_matrix(src, r.BRIGHTNESS);
  return rope_pattern.map_mono(w, h);
}

public PGraphics pattern_marble_brightness(PImage src, int w, int h) {
  init_pattern();
  rope_pattern.build_matrix(src, r.BRIGHTNESS);
  return rope_pattern.marble_mono(w, h);
}

public PGraphics pattern_marble_rgb(PImage src, int w, int h) {
  init_pattern();
  rope_pattern.build_matrix(src, RGB);
  return rope_pattern.marble_xyz(w, h);
}

public PGraphics pattern_marble_hsb(PImage src, int w, int h) {
  init_pattern();
  rope_pattern.build_matrix(src, HSB);
  return rope_pattern.marble_xyz(w, h);
}

public PGraphics pattern_marble(int w, int h) {
  init_pattern();
  rope_pattern.build_matrix_rand_mono();
  return rope_pattern.marble_mono(w, h);
}
...
public vec2 follow(vec2 target, float speed, vec2 buf) {
  return follow(target.x(), target.y(), speed, speed, buf);
}

public vec2 follow(vec2 target, vec2 speed, vec2 buf) {
  return follow(target.x(), target.y(), speed.x(), speed.y(), buf);
}

public vec2 follow(float tx, float ty, float speed, vec2 buf) {
  return follow(tx, ty, speed, speed, buf);
}

public vec2 follow(float tx, float ty, float sx, float sy, vec2 buf) {
...
public void polyhedron(String type, int style, int size) {
  polyhedron(type,style,size,null);
}

public void polyhedron(String type, int style, int size, PGraphics other) {
...
*/
public boolean edge_is(int vID1, int vID2, ArrayList<vec3> array_points) {
...
/////////////////////////////////
public void add_permutations(float x, float y, float z) {
...
public void add_vertices(float x, float y, float z) {
...
* tetrahedron
*/
public void tetrahedron_draw_point(vec3 point, PGraphics other) {
  push(other);
  rotateX(TAU -1,other);
  rotateY(PI/4,other);
  point(point.mult(factor_size_polyhedron));
...
public void tetrahedron_draw_line(vec3 v1, vec3 v2, PGraphics other) {
  push(other) ;
  rotateX(TAU -1,other);
  rotateY(PI/4,other);
  line(v1.mult(factor_size_polyhedron), v2.mult(factor_size_polyhedron), other);
...
public void tetrahedron_draw_vertex(PGraphics other) {
  push(other);
  rotateX(TAU -1,other);
  rotateY(PI/4,other) ;
...
public void print_list_costume() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "generateur_de_monde" };
    if (passedArgs != null) {
