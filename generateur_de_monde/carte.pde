...
void map_generator(boolean show_info) {
	urbanist();
	vec3 area = new vec3(10);
	vec6 canvas_birth = new vec6(0, 0, -width, width, height, width);
	// boolean show_info = true;


	if(r.compare(urbanist.get_pos(),urbanist.get_destination(),area)) {
...
/**
* count segment meeting
*/
int count_segment_meeting = 0;
void set_count_segment_meeting(int value) {
	count_segment_meeting = value;
}

void add_count_segment_meeting(int value) {
	count_segment_meeting += value;
}

int get_count_segment_meeting() {
	return count_segment_meeting;
}
...
boolean check_meeting_segment(R_Segment target_segment, ArrayList<R_Segment> seg_list, boolean show_info) {
	...
	for(R_Segment s : seg_list) {
  	...
  	if(target_segment.meet_is(s) && get_count_segment_meeting() < max_iter_for_meeting) {
  		add_count_segment_meeting(1);
  		is = true;
  		break;
  	}
  }
